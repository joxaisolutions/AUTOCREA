...
export function PricingCardSimple({ plan, currentPlanId }: PricingCardSimpleProps) {
  ...
  const getButtonHref = () => {
    ...
    if (isFree) return '/sign-up';
    if (isEnterprise) return 'mailto:sales@autocrea.joxai.org';
    return '/pricing';
  };

  return (
    <Card 
      className={`relative flex flex-col h-full transition-all duration-300 hover:scale-105 ${
        plan.popular 
    ...
    >
      ...
      <CardContent className="flex-grow flex flex-col pt-4">
        ...
        <Link href={getButtonHref()} className="w-full">
          <Button
